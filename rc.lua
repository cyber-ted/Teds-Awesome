...
for s = 1, screen.count() do
    ...
    tags[s] = awful.tag({ "DEV", "WWW", "SYS", "DOC", "VBOX", "VIM", "MUS", "VID", "GFX" }, s, layouts[1])
end
-- }}}
...
for s = 1, screen.count() do
    ...
    -- Create the wibox
    mywibox[s] = awful.wibox({ position = "left",orientaion = "north", screen = s })
    twobox[s] = awful.wibox( {position = "top", screen = s})
   
    -- LEFT WIBOX 2 (TOP BAR)
    local left_layout = wibox.layout.fixed.horizontal()
    left_layout:add(mypromptbox[s])

    -- MIDDLE WIBOX 2 (TOP BAR)
    local middle_layout = wibox.layout.fixed.horizontal()
    middle_layout:add(mytaglist[s])

    -- RIGHT WIBOX 2 (TOP BAR)
    local right_layout = wibox.layout.fixed.horizontal()
    right_layout:add(mylauncher)

    -- BRINGING IT ALL TOGETHER WIBOX 2 (TOP BAR)
    local new_layout = wibox.layout.align.horizontal()
    new_layout:set_left(left_layout)
    new_layout:set_middle(middle_layout)
    new_layout:set_right(right_layout)	
    
    -- BOTTOM WIBOX 1 (LEFT BAR)
    ...
    -- WIBOX 2 (TOP BAR)
    twobox[s]:set_widget(new_layout)
end
-- }}}
